...
const transform: AxiosTransform = {
  ...
  transformResponseHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    const downloadFile = (data, name, type = '') => {
      try {
        ...
        if (type === '') {
          ...
          if (newName === 'xls' || newName === 'xlsx') {
            blob = new Blob([data], { type: 'application/vnd.ms-excel' });
          } else if (newName === 'audio/mpeg') {
    ...
    if (
      ...
      const contentArr = contentDisposition.split('filename=');
      let fileName = contentArr[1];
      if (fileName === '') {
        return;
      }
  ...
  beforeRequestHook: (config, options) => {
    ...
    if (config.method?.toUpperCase() === RequestEnum.GET) {
      if (!isString(params)) {
        ...
        config.params = userInfo.customerId
      ...
      if (!isString(params)) {
        ...
        if (
          ...
          if (config.data instanceof FormData) {
            ...
            config.data = userInfo.customerId
              ? { ...config.data, ...{ customerId: userInfo.customerId } }
              : config.data;
          }
...
export const selfHttp = createAxios({
  requestOptions: {
