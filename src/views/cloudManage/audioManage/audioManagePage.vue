<template>
  <div>
    <Loading :loading="data.loading" tip="加载中，请勿关闭页面！" />
...
<script setup>
  ...
  import { audioPage, fileExport, logLook, logDownload } from '/@/api/manage/manage';
  ...
  const customerId = computed(() => userStore.getUserInfo.id);
  const getList = async () => {
  ...
  const batchAdd = async (val) => {
    if (val) {
      if (!checkedKeys.value.length) {
        ...
        let res = await fileExport({ fileIds: checkedKeys.value.join(','), type: 1 }); // 0-日志，1-音频
      ...
      let res = await fileExport({ ...getForm().getFieldsValue(), type: 1 }); //0-日志，1-音频
  ...
  const play = async (record) => {
    let res = await logLook({ storeType: record.storeType, storeUrl: record.storeUrl, type: 1 });
    if (res.code == 0) {
