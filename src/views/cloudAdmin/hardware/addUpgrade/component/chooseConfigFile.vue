...
<script setup>
  ...
  import { chooseConfigForm, chooseConfigColumn } from '../schemas.ts';
  import { resourcePage } from '/@/api/admin/admin';
  import { defineEmits, inject, nextTick, reactive } from 'vue';
  const emit = defineEmits(['ChooseConfigRes', 'register']);
  ...
  const handleAdd = async (record) => {
    emit('ChooseConfigRes', record);
    closeModal();
  };
  const handleDelete = async () => {
    emit('ChooseConfigRes', { id: undefined });
  };
