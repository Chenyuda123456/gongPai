<template>
  <BasicDrawer v-bind="$attrs" @register="register" :isDetail="true" title="门店升级">
    <div class="p-16px">
      <BasicTable
        ...
        :title="`设备类型：${data.record.deviceType} | ${
          $route.path.includes('admin-hardware/hardwareList')
            ...
            : ''
        } 升级范围：按客户门店 ，以下为要升级的门店列表（合计 ${data.statistic.shopNum} 家，${
    ...
    <rangeDeviceUpgrade :title="'门店'" @reload="getList" @register="registerDrawerRangeDevice" />
  </BasicDrawer>
</template>
<script setup lang="ts">
  ...
  import { shopColumn } from './schemas.ts';
