<template>
  <BasicDrawer v-bind="$attrs" @register="register" :isDetail="true" title="固件升级结果">
    <div>
      <div class="flex mb-0 bg-white">
        <div class="left">
          ...
          <div class="content flex">
            ...
            <div class="item">{{ data.record.version }}</div>
          </div>
        </div>
...
<script setup lang="ts">
  ...
  import { resultCount, resultPage, taskAgain, otaResultExport } from '/@/api/admin/admin';
  ...
  const getList = async () => {
    ...
    let res = await resultPage({
      ...
      ...formData,
      taskId: data.record.taskId,
      resourceType: 0, // 升级类型 0-固件；1-配置单
    });
    if (res.code == 0) {
